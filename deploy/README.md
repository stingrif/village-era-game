...
|------------|--------|----------|
| `EDITOR_API_KEY` | tigrit-api | Ключ редактора карты (`X-API-Key` в PUT /api/map) |
| `TIGRIT_ADMIN_API_KEY` | tigrit-api | Ключ Admin API (`X-Admin-Key`) |
| `CORS_ORIGINS` | tigrit-api | Включить `https://tigrit.stakingphxpw.com` |

Добавить в `бэкенд/.env`:
```bash
...
### Миграции БД (обязательно перед первым деплоем)
...
При повторном запуске — без ошибок (`ADD COLUMN IF NOT EXISTS` идемпотентен).

### Вкладка Админ

- Открыть `https://tigrit.stakingphxpw.com` → вкладка 🛡️ Админ
- Ввести `TIGRIT_ADMIN_API_KEY` в поле «Admin API Key»
- Форма автозаполняется данными деревни
- Кнопки disabled без ключа
- Быстрые активации: завершить стройку, залить ресурсы, поднять уровень

Полная документация: `Инструкция/30_Admin_API_Тигрит.md`
